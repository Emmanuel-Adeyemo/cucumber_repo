...
the whole system collapse. A better logic is to isolate the plotting, preprocessing, training, and 
...
of used packages and dependencies are captured, ensuring that the result from my local machine is 
the same as what comes back from the cloud.
...
4. Event driven approach: I can configure the pipeline so that a run is triggered by event for example when a new data drops
into the low tier storage bucket, it can be a signal to initiate the workflow. It is also important to continue to monitor
metrics. E.g. an alert might be sent when metric drift below a particular threshold to signal further investigation.
5. Use CI/CD for maintenance: code changes pushed to the repository automatically triggers a new docker build.
This allows for version control as well as ensuring the workflow uses the latest code improvements.
